...
namespace Features.Map
{
    ...
    public class MapProducer
    {
        ...
        public Entity.Map Generate(int rows, int columns, int countDistanceCard, int countCoinCard)
        {
            var map = _repository.FindCurrent();
            var deck = CreateDeck(rows * columns, countDistanceCard, countCoinCard);
            var idx = 0;
        ...
        private static CardType[] CreateDeck(int countAll, int countDistanceCard, int coinCard)
        {
            if (countAll < countDistanceCard + coinCard + 1)
